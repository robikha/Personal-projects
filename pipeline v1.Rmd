...
```{python}
BS = BS_T_df.loc[BS_T_df['PartyId'] == companies_s[0][0]].T.to_numpy()
PnL = PnL_T_df.loc[PnL_T_df['PartyId'] == companies_s[0][0]].T.to_numpy()

nrows, ncols = np.shape(BS)

#row_header =['PartyId', 'PeriodEndDate', 'CostOfSalesValue', 'OperatingExpenditureValue', 'OtherExpensesValue', 'ProfitGrossValue','ProfitNetValue', 'ProfitOperatingValue', 'TaxValue', 'RevenueValue', 'Month' ]


print(BS)


#PnL_T_df.reindex(["Z", "C", "A"])
```
...
#np.shape(months)
PnL_36 = np.zeros([len(row_header)-4, 36])
PnL_st = set(PnL[-1,:])
PnL_36_cols= {e:i for i, e in enumerate(months) if e in PnL_st}
PnL_36[:, list(PnL_36_cols.values())] = PnL[2:9, :]
...
```{python}
conn = sqlite3.connect('TestDB9.db') 
c = conn.cursor()
